...
import org.usfirst.frc.team291.auto.arrays.NearScaleSidePath;
...
public class NearSideOneCubeSpecialScaleAuto extends AutoMode{
	...
	public NearSideOneCubeSpecialScaleAuto(boolean startOnLeft){
		System.out.println("Starting NearSideOneCubeScaleAuto");
		timer = new Timer();
		this.startOnLeft = startOnLeft;
		if(startOnLeft){
			trajectoryLeft = NearScaleSidePath.trajectoryArray[0];
			trajectoryRight = NearScaleSidePath.trajectoryArray[1];
		}
		else {
			trajectoryLeft = NearScaleSidePath.trajectoryArray[1];
			trajectoryRight = NearScaleSidePath.trajectoryArray[0];
		}
	...
	public void execute() {
		...
		case FINISHED:
			...
			if(driveBase.gyroIsConnected() && timer.get() > 2){
				driveBase.turnToAngle(190.0);
			}
			//driveBase.setLeftRightPower(0,0);
			break;
		}
